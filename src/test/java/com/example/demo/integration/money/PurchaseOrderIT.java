...
class PurchaseOrderIT {
  ...
  void admin_can_filter_purchase_orders_by_job_id() throws Exception {
    ...
    assertTrue(
        result.stream().allMatch(po -> po.getJob() != null && JOB1_ID.equals(po.getJob().getId())));
  }
  ...
  void admin_can_create_purchase_order() throws Exception {
    ...
    assertEquals(
        toCreate.getSupplierId(),
        created.get(0).getSupplier() != null ? created.get(0).getSupplier().getId() : null);
  }
